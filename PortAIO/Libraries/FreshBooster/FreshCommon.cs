...
namespace FreshBooster
{
    ...
    class FreshCommon
    {
        ...
        public static LeagueSharp.Common.HitChance Hitchance(string Type)
        {
            LeagueSharp.Common.HitChance result = LeagueSharp.Common.HitChance.Low;
            switch (PortAIO.Champion.Blitzcrank.Program.getQHitChance())
            {
                case 1:
                    result = LeagueSharp.Common.HitChance.OutOfRange;
                    break;
                case 2:
                    result = LeagueSharp.Common.HitChance.Impossible;
                    break;
                case 3:
                    result = LeagueSharp.Common.HitChance.Low;
                    break;
                case 4:
                    result = LeagueSharp.Common.HitChance.Medium;
                    break;
                case 5:
                    result = LeagueSharp.Common.HitChance.High;
                    break;
                case 6:
                    result = LeagueSharp.Common.HitChance.VeryHigh;
                    break;
            }
            return result;
        }
        ...
        {
            var CusPos = Pos;
            if (ObjectManager.Player.Distance(CusPos) > range) CusPos = ObjectManager.Player.Position.Extend(Game.CursorPos, range).To3D();
