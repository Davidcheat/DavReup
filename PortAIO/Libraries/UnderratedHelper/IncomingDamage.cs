    ...
    internal class IncomingDamage
    {
        ...
        private void CheckSkillShots()
        {
            if (!skillShotChecked)
            {
                ...
                LeagueSharp.Common.Utility.DelayAction.Add(70, () => skillShotChecked = false);
                for (int i = 0; i < SkillshotDetector.ActiveSkillshots.Count; i++)
                {
                    if (
                        CombatHelper.BuffsList.Any(b => SkillshotDetector.ActiveSkillshots[i].SkillshotData.Slot == b.Slot && ((AIHeroClient) SkillshotDetector.ActiveSkillshots[i].Caster).ChampionName == b.ChampionName))
                    {
                            ...
                            .OrderBy(h => h.Distance(SkillshotDetector.ActiveSkillshots[i].Caster)))
                    {
                                ...
                                350, Hero, SkillshotDetector.ActiveSkillshots[i].Caster))
                        {
                            ...
                            var newData = new Dmg(
                                Hero,
                                (float)
                                    LeagueSharp.Common.Damage.LSGetSpellDamage(
                    ...
                    if (sender.IsValid && !sender.IsDead)
                    {
                        ...
                        if (data != null)
                        {
                            ...
                            if (Orbwalking.IsAutoAttack(args.SData.Name))
                            {
                                ...
                                data.Damages.Add(new Dmg(target, dmg, missileSpeed, !sender.Name.ToLower().Contains("turret")));
                            }
                            else
                            {
                                        ...
                                        b => args.Slot == b.Slot && hero.ChampionName == b.ChampionName))
                                {
                                    data.Damages.Add(
                                        new Dmg(
                                            target,
                                            (float) LeagueSharp.Common.Damage.LSGetSpellDamage(hero, (Obj_AI_Base) args.Target, args.Slot),
                                            missileSpeed, false,
