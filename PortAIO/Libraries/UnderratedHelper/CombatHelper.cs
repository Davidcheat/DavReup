...
namespace UnderratedAIO.Helpers
{
    public class CombatHelper
    {
        ...
        public static LeagueSharp.Common.HitChance GetHitChance(int qHit)
        {
            var hitC = LeagueSharp.Common.HitChance.High;
            switch (qHit)
            {
                case 1:
                    hitC = LeagueSharp.Common.HitChance.Low;
                    break;
                case 2:
                    hitC = LeagueSharp.Common.HitChance.Medium;
                    break;
                case 3:
                    hitC = LeagueSharp.Common.HitChance.High;
                    break;
                case 4:
                    hitC = LeagueSharp.Common.HitChance.VeryHigh;
                    break;
            }
            return hitC;
        }
        ...
        public static bool IsFacing(Obj_AI_Base source, Vector3 target, float angle = 90)
        {
            ...
            return
                (double)
                    LeagueSharp.Common.Geometry.LSAngleBetween(
                        LeagueSharp.Common.Geometry.LSPerpendicular(LeagueSharp.Common.Geometry.LSTo2D(source.Direction)), LeagueSharp.Common.Geometry.LSTo2D(target - source.Position)) <
                angle;
        }
        ...
        public static double GetAngle(Obj_AI_Base source, Vector3 target)
        {
            ...
            return LeagueSharp.Common.Geometry.LSAngleBetween(
                LeagueSharp.Common.Geometry.LSPerpendicular(LeagueSharp.Common.Geometry.LSTo2D(source.Direction)), LeagueSharp.Common.Geometry.LSTo2D(target - source.Position));
            ;
        }
        ...
        public static LeagueSharp.Common.Geometry.Polygon GetPoly(Vector3 pos, float range, float widht)
        {
            ...
            var pos4 =
                (POS + (POS - player.ServerPosition.To2D()).Normalized() -
                 direction.Perpendicular() * widht / 2f).To3D();
            var poly = new LeagueSharp.Common.Geometry.Polygon();
            poly.Add(pos1);
            poly.Add(pos3);
            poly.Add(pos2);
            poly.Add(pos4);
        ...
        public static LeagueSharp.Common.Geometry.Polygon GetPolyFromVector(Vector3 from, Vector3 to, float width)
        {
                ...
                if (t < 0.5)
                {
                    switch (enemy.ChampionName)
                    {
                        case "Ahri":
                            ...
                            {
                                result += (LeagueSharp.Common.Damage.LSGetSpellDamage(enemy, player, spell.Slot));
                                result += (LeagueSharp.Common.Damage.LSGetSpellDamage(enemy, player, spell.Slot, 1));
                            }
                            else
                            {
                                result += LeagueSharp.Common.Damage.LSGetSpellDamage(enemy, player, spell.Slot);
                        ...
                        case "Akali":
                            ...
                            {
                                result += (LeagueSharp.Common.Damage.LSGetSpellDamage(enemy, player, spell.Slot) * spell.Ammo);
                            }
                        ...
                        case "Amumu":
                            ...
                            {
                                result += (LeagueSharp.Common.Damage.LSGetSpellDamage(enemy, player, spell.Slot) * 5);
                            }
                            else
                            {
                                result += LeagueSharp.Common.Damage.LSGetSpellDamage(enemy, player, spell.Slot);
                        ...
                        case "Cassiopeia":
                            ...
                            {
                                result += (LeagueSharp.Common.Damage.LSGetSpellDamage(enemy, player, spell.Slot) * 2);
                            }
                            else
                            {
                                result += LeagueSharp.Common.Damage.LSGetSpellDamage(enemy, player, spell.Slot);
                        ...
                        case "Garen":
                            ...
                            {
                                result += (LeagueSharp.Common.Damage.LSGetSpellDamage(enemy, player, spell.Slot) * 3);
                            }
                            else
                            {
                                result += LeagueSharp.Common.Damage.LSGetSpellDamage(enemy, player, spell.Slot);
                        ...
                        case "Irelia":
                            if (spell.Slot == SpellSlot.W)
                            {
                                result += (LeagueSharp.Common.Damage.LSGetSpellDamage(enemy, player, spell.Slot) * attacks);
                            }
                            else
                            {
                                result += LeagueSharp.Common.Damage.LSGetSpellDamage(enemy, player, spell.Slot);
                        ...
                        case "Karthus":
                            if (spell.Slot == SpellSlot.Q)
                            {
                                result += (LeagueSharp.Common.Damage.LSGetSpellDamage(enemy, player, spell.Slot) * 4);
                            }
                            else
                            {
                                result += LeagueSharp.Common.Damage.LSGetSpellDamage(enemy, player, spell.Slot);
                        ...
                        case "Rammus":
                            if (spell.Slot == SpellSlot.R)
                            {
                                result += LeagueSharp.Common.Damage.LSGetSpellDamage(enemy, player, spell.Slot) * 6;
                            }
                            else
                            {
                                result += LeagueSharp.Common.Damage.LSGetSpellDamage(enemy, player, spell.Slot);
                        ...
                        case "Viktor":
                            ...
                            {
                                result += LeagueSharp.Common.Damage.LSGetSpellDamage(enemy, player, spell.Slot);
                                result += LeagueSharp.Common.Damage.LSGetSpellDamage(enemy, player, spell.Slot, 1) * 5;
                            }
                            else
                            {
                                result += LeagueSharp.Common.Damage.LSGetSpellDamage(enemy, player, spell.Slot);
        ...
        internal static int CountEnemiesInRangeAfterTime(Vector3 pos, float range, float delay, bool nowToo)
        {
            var enemies = (from h in HeroManager.Enemies
                let pred = LeagueSharp.Common.Prediction.GetPrediction(h, delay)
    ...
    public class BuffData
    {
        ...
        public double GetdTotalBuffDamage(Obj_AI_Base target, BuffInstance buff)
        {
            ...
            {
                return LeagueSharp.Common.Damage.LSGetSpellDamage(caster, target, Slot, Stage) * Multiplier;
            }
            if (BuffName == "toxicshotparticle")
            {
                var dmg = new double[] { 24, 48, 72, 96, 120 }[SpelLevel - 1] + 0.4 * caster.TotalMagicalDamage;
                return LeagueSharp.Common.Damage.CalcDamage(caster, target, DamageType.Magical, dmg);
            }
            ...
            if (BuffName == "dariushemo")
            {
                ...
                return LeagueSharp.Common.Damage.CalcDamage(caster, target, DamageType.Physical, dmg);
            }
            ...
            {
                var dmg = LeagueSharp.Common.Damage.LSGetSpellDamage(caster, target, Slot, Stage) * buff.Count;
                return LeagueSharp.Common.Damage.CalcDamage(caster, target, DamageType.Physical, dmg);
            }
            if (BuffName == "swainbeamdamage")
            {
                var dmg = LeagueSharp.Common.Damage.LSGetSpellDamage(caster, target, Slot, Stage) * 3;
                return LeagueSharp.Common.Damage.CalcDamage(caster, target, DamageType.Magical, dmg);
            }
