namespace KoreanCommon
{
    ...
    public class CommonForceUltimate
    {
        ...
        private bool UltimateUp()
        {
            ...
            if (CommonMenu.getForceUlt() && Orbwalker.ActiveModesFlags.HasFlag(Orbwalker.ActiveModes.Combo))
            {
        ...
        private void CheckMouseButtons(WndEventArgs args)
        {
            if (UltimateUp())
            {
                ...
                {
                    if (TargetSelector.GetTarget(champion.Spells.R.Range, DamageType.Magical) == null)
                    {
