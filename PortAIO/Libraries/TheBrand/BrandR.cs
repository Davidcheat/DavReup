...
namespace TheBrand
{
    class BrandR : Skill // 550 bounce range
    {
        ...
        public override void Execute(AIHeroClient target)
        {
            ...
            {
                if (ObjectManager.Player.HealthPercent - target.HealthPercent < PortAIO.Champion.Brand.Program.getRMenuSL("healthDifference") || !PortAIO.Champion.Brand.Program.getRMenuCB("DontRwith") || PortAIO.Champion.Brand.Program.getRMenuCB("Ignorewhenfleeing") && target.Distance(ObjectManager.Player) > ObjectManager.Player.AttackRange)
                {
                    Cast(target);
                }
            ...
            if (canBounce)
            {
                ...
                if ((inBounceEnemies.Any(enemy => (dmgPerBounce > enemy.Health && MaxDamage > enemy.Health)) && (bounceCount == 2 || PortAIO.Champion.Brand.Program.getRMenuCB("RiskyR"))))
                {
                    TryUlt(target, inBounceEnemies, distance);
                }
                else if (bounceCount == 2 && dmgPerBounce * 3 > target.Health && MaxDamage > target.Health && distance < 750 && PortAIO.Champion.Brand.Program.getRMenuCB("RiskyR"))
                {
                    Cast(target);
                }
                ...
                else if (PortAIO.Champion.Brand.Program.getRMenuCB("Ultnonkillable") && PortAIO.Champion.Brand.Program.getRMenuSL("whenminXtargets") <= bounceCount)
                {
        ...
        {
            if (!PortAIO.Champion.Brand.Program.getRMenuCB("BridgeR")) return;
            #region bridge ult
