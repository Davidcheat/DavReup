...
namespace TheBrand.Commons.ComboSystem
{
    public class ComboProvider
    {
        ...
        public ComboProvider(float targetSelectorRange, IEnumerable<Skill> skills)
        {
            ...
            DamageType = Skills.Count(spell => spell.DamageType == DamageType.Magical) > Skills.Count(spell => spell.DamageType == DamageType.Physical) ? DamageType.Magical : DamageType.Physical;
        ...
        /// Represents a "combo" and it's logic. Manages skill logic.
        /// </summary>
        public ComboProvider(float targetSelectorRange, params Skill[] skills) : this(targetSelectorRange, skills.ToList()) { }

        #region Menu creators
        public void CreateBasicMenu(Menu comboMenu, Menu harassMenu, Menu laneclearMenu, bool laneclearHarassSwitch = true)
        {
            if (comboMenu != null)
            {
        ...
        public void CreateComboMenu(Menu comboMenu, params SpellSlot[] forbiddenSlots)
        {
            foreach (var skill in Skills)
            {
                ...
                {
                    comboMenu.Add(skill.Slot + "Hitchance", new Slider(skill.Slot + " Hitchance (0 : Low | 1 : Medium | 2 : High | 3 : Very High)", 3, 0, 3));
                }
            }
        }
        ...
        public void CreateHarassMenu(Menu harassMenu, params SpellSlot[] forbiddenSlots)
        {
            foreach (var skill in Skills)
            {
                ...
                if (skill.IsSkillshot)
                {
                    harassMenu.Add(skill.Slot + "Hitchance", new Slider(skill.Slot + " Hitchance (0 : Low | 1 : Medium | 2 : High | 3 : Very High)", 3, 0, 3));
                }
            }
        }
