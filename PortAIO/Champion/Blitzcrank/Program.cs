...
namespace PortAIO.Champion.Blitzcrank
{
    class Program
    {
        ...
        private static void SkillSet()
        {
            try
            {
                _Q = new LeagueSharp.Common.Spell(SpellSlot.Q, 950f);
                _Q.SetSkillshot(0.25f, 70f, 1800f, true, SkillshotType.SkillshotLine);
                _W = new LeagueSharp.Common.Spell(SpellSlot.W, 700f);
                _E = new LeagueSharp.Common.Spell(SpellSlot.E, 150f);
                _R = new LeagueSharp.Common.Spell(SpellSlot.R, 540f);
            }
            catch (Exception)
            {
        ...
        private static void Menu()
        {
            try
            {
                ...
                HarassMenu.Add("Blitzcrank_AManarate", new Slider("Mana %", 20, 0, 100));
                ...
                foreach (var enemy in ObjectManager.Get<AIHeroClient>())
                {
                    ...
                    {
                        MiscMenu.Add("Blitzcrank_GrabSelect" + enemy.ChampionName, new Slider("Grab Mode (0 : Enable | 1 : Don't | 2 : Auto) " + enemy.ChampionName, 0, 0, 2));
        ...
        public static void Drawing_OnEndScene(EventArgs args)
        {
            try
            {
                ...
                {
                    if (getCheckBoxItem(DrawMenu, "Blitzcrank_Indicator"))
                    {
        ...
        private static void OnGameUpdate(EventArgs args)
        {
            try
            {
                ...
                if (getCheckBoxItem(MiscMenu, "Blitzcrank_GrabDash") && _Q.IsReady())
                    if (QTarget != null && _Q.GetPrediction(QTarget, false).Hitchance == LeagueSharp.Common.HitChance.Dashing)
                        _Q.CastIfHitchanceEquals(QTarget, LeagueSharp.Common.HitChance.Dashing, true);

                //killsteal
                if (getCheckBoxItem(KSMenu, "Blitzcran_KUse_Q") && QTarget != null && QTarget.Health < _Q.GetDamage(QTarget) && _Q.IsReady())
                {
                    _Q.CastIfHitchanceEquals(QTarget, LeagueSharp.Common.HitChance.VeryHigh, true);
                    return;
                }
                if (getCheckBoxItem(KSMenu, "Blitzcran_KUse_R") && RTarget != null && RTarget.Health < _E.GetDamage(RTarget) && _R.IsReady())
                {
                    _R.Cast(true);
                    return;
                }
                ...
                foreach (var enemy in ObjectManager.Get<AIHeroClient>())
                {
                    if (enemy.Team != Player.Team && QTarget != null && getSliderItem(MiscMenu, "Blitzcrank_GrabSelect" + enemy.ChampionName) == 2 && _Q.IsReady() && QTarget.ChampionName == enemy.ChampionName)
                    {
                ...
                {
                    if (getCheckBoxItem(ComboMenu, "Blitzcrank_CUse_Q") && _Q.IsReady() && QTarget != null && getSliderItem(MiscMenu, "Blitzcrank_GrabSelect" + QTarget.ChampionName) != 1)
                    {
                        _Q.CastIfHitchanceEquals(QTarget, FreshCommon.Hitchance("Blitzcrank_CUseQ_Hit"), true);
                    }
                    if (getCheckBoxItem(ComboMenu, "Blitzcrank_CUse_W") && _W.IsReady() && WTarget != null)
                        _W.Cast(Player, true);
                    if (getCheckBoxItem(ComboMenu, "Blitzcrank_CUse_E") && _E.IsReady() && QTarget.Distance(Player.ServerPosition) < 230)
                        _E.Cast(Player);
                ...
                // Harass
                if (Orbwalker.ActiveModesFlags.HasFlag(Orbwalker.ActiveModes.Harass) && getSliderItem(HarassMenu, "Blitzcrank_AManarate") < Player.ManaPercent)
                {
                    if (getCheckBoxItem(HarassMenu, "Blitzcrank_HUse_Q") && _Q.IsReady() && QTarget != null && getSliderItem(MiscMenu, "Blitzcrank_GrabSelect" + QTarget.ChampionName) != 1)
                    {
                        _Q.CastIfHitchanceEquals(QTarget, FreshCommon.Hitchance("Blitzcrank_CUseQ_Hit"), true);
                    }
                    if (getCheckBoxItem(HarassMenu, "Blitzcrank_HUse_W") && _W.IsReady() && WTarget != null)
                        _W.Cast(Player, true);
                    if (getCheckBoxItem(HarassMenu, "Blitzcrank_HUse_E") && _E.IsReady() && QTarget.Distance(Player.ServerPosition) < 230)
                        _E.Cast(Player);
                }
            }
            catch (Exception)
