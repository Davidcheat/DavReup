namespace ElUtilitySuite.Items
{
    ...
    public class Zhonya : IPlugin
    {
        ...
        public void CreateMenu(Menu rootMenu)
        {
            ...
            zhonyaMenu.Add("ZhonyaHPSlider", new Slider("HP Percent", 10, 1, 50));
            zhonyaMenu.Add("NoZhonyaEvade", new CheckBox("Don't Zhonya if spell is evade-able (BETA)", false));
        }
        ...
        private void GameObjectOnCreate(GameObject sender, EventArgs args)
        {
            if (!sender.IsValid<MissileClient>() || sender.IsAlly || !zhonyaItem.IsReady()
                || !getCheckBoxItem("ZhonyaDangerous"))
            {
                return;
            }
            ...
            if (missile.SData.LineWidth + Player.BoundingRadius
                > Player.ServerPosition.To2D()
                      ...
                      .SegmentPoint))
            {
                if (!(getCheckBoxItem("NoZhonyaEvade") && this.CanEvadeMissile(missile, Player)))
                {
                    zhonyaItem.Cast();
                }
            }
        ...
        private void ObjAiBaseOnProcessSpellCast(Obj_AI_Base sender, GameObjectProcessSpellCastEventArgs args)
        {
                ...
                || (!isLinear && Player.Distance(endPosition) <= width + Player.BoundingRadius))
            {
                ...
                if (getCheckBoxItem("NoZhonyaEvade") && spellData.MissileName != null)
                {
                    return;
                }
