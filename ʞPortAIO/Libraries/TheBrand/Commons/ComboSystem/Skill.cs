...
{
    public abstract class Skill : LeagueSharp.Common.Spell, IComparable<Skill>
    {
        public LeagueSharp.Common.HitChance MinComboHitchance = LeagueSharp.Common.HitChance.Low, MinHarassHitchance = LeagueSharp.Common.HitChance.VeryHigh;
        ...
        public virtual void Update(Orbwalker.ActiveModes mode, ComboProvider combo, AIHeroClient target)
        {
            ...
            if (OnlyUpdateIfTargetValid && (mode == Orbwalker.ActiveModes.Combo || mode == Orbwalker.ActiveModes.Harass) && !target.IsValidTarget()) return;
            ...
            {
                if (LaneclearEnabled && ObjectManager.Player.ManaPercent > PortAIO.Champion.Brand.Program.getMiscMenuSL("manaLC"))
                {
            ...
            {
                if (HarassEnabled && ObjectManager.Player.ManaPercent > PortAIO.Champion.Brand.Program.getMiscMenuSL("manaH"))
                {
                    Harass(combo, target);
                }
            }
        ...
        {
            MinComboHitchance = hitchance.ToEnum<LeagueSharp.Common.HitChance>();
        }
        ...
        {
            MinHarassHitchance = hitchance.ToEnum<LeagueSharp.Common.HitChance>();
        }
        ...
        /// <returns></returns>
        public bool CouldHit(Obj_AI_Base unit, LeagueSharp.Common.HitChance minHitChance = LeagueSharp.Common.HitChance.Low, bool aoe = false)
        {
